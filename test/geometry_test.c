...
void Line2MutateTest() {
  ...
  Line2RotateAroundPoint(&a, &b, rotate_point, F32_PI / 2.0f);
...
void Line2QueryTest() {
  ...
  V2 normal;
  a = V2Assign(0, 0); b = V2Assign(1, 0);
  normal = Line2GetNormalIn(a, b);
  EXPECT_V2_APPROX_EQ(normal, V2Assign(0, 1));
  normal = Line2GetNormalOut(a, b);
  EXPECT_V2_APPROX_EQ(normal, V2Assign(0, -1));
...
void Line2IntersectRay2Test() {
  ...
  ray_start = V2Assign(5, 5);
  ray_dir   = V2Normalize(V2Assign(-1, -1));
  EXPECT_TRUE(Line2IntersectRay2(line0, line1, ray_start, ray_dir, &ip));
...
void Ray2RotateAroundPointTest() {
  V2 start, dir, point;

  start = V2Assign(0, 0); dir = V2Assign(1, 0);
  point = V2Assign(0, 0);
  Ray2RotateAroundPoint(&start, &dir, point, F32_PI / 2.0f);
...
void Ray2IntersectRay2Test() {
  ...
  a0 = V2Assign(5, 5); ad = V2Normalize(V2Assign(1, 1));
  b0 = V2Assign(0, 0); bd = V2Normalize(V2Assign(-1, -1));
...
void Tri2MutateTest() {
  ...
  Tri2RotateAroundPoint(tri, V2Splat(0), F32_PI / 2.0f);
  ...
  V2 computed_center = Tri2GetCenter(tri);
...
void Aabb2QueryTest() {
  ...
  EXPECT_F32_APPROX_EQ(r, V2Length(V2Assign(2, 2)));
...
void Aabb2RotateAroundPointTest() {
  V2 c = V2Assign(2,0);
  V2 s = V2Assign(4,4);
  V2 pivot = V2Assign(0,0);

  Aabb2RotateAroundPoint(&c, pivot, F32_PI/2.0f);
...
void Obb2RotateAroundPointTest() {
  V2 c = V2Assign(2,0);
  F32 ang = 0.0f;
  V2 pivot = V2Assign(0,0);

  // NOTE: rotate 90° CCW around origin
  Obb2RotateAroundPoint(&c, &ang, pivot, F32_PI/2.0f);
...
void Obb2GetEnclosingCircle2Test() {
  ...
  EXPECT_F32_APPROX_EQ(r, V2Length(V2Assign(2, 3)));
}
...
void ConvexHull2RotateAroundPointTest() {
  V2 h[3], p;
  ...
  ConvexHull2RotateAroundPoint(h,3,p,F32_PI/2.0f);
...
void Ray3IntersectConvexHull3Test() {
  ...
  ConvexHull3FromAabb3(hull_points, hull_indices, V3Splat(0), hull_size);
...
void Plane3IntersectPlane3Test() {
  ...
  b_normal = V3Normalize(V3Assign(1, 0, 1));
  b_d = 4.0f / V3Length(b_normal);
  EXPECT_TRUE(Plane3IntersectPlane3(a_normal, a_d, b_normal, b_d, &intersect_point, &intersect_dir));
  V3 expected_dir = V3CrossV3(a_normal, b_normal);
...
int main(void) {
  ...
  RUN_TEST(Ray2EqTest);
  RUN_TEST(Ray2RotateAroundPointTest);
  ...
  RUN_TEST(Aabb2RotateAroundPointTest);
  ...
  RUN_TEST(Obb2EqTest);
  RUN_TEST(Obb2RotateAroundPointTest);
  ...
  RUN_TEST(ConvexHull2RotateAroundPointTest);
